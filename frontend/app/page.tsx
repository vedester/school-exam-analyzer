...
import { 
  ...
  BookOpen, LayoutList, FileQuestion, Play, ArrowLeft
} from "lucide-react";
...
export default function Home() {
  ...
  // Controls updating the sidebar list
  ...
  const loadDemoData = async () => {
    try {
      ...
      if(!response.ok) throw new Error("Demo file not found on server.");
      ...
      console.error(error);
      alert("Could not load demo data. Please make sure sample_exam.xlsx is in the public folder.");
      setLoading(false);
    }
  };
  ...
  const updateRule = (index: number, field: keyof GradingRule, value: string | number) => {
    ...
    // @ts-expect-error: dynamic assignment for simplicity
  ...
  // FIX: Replaced 'any' with 'ExamResult' to silence the linter
  const handleHistorySelect = (exam: ExamResult) => {
    setResultData(exam);
    setStatus("completed");
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center p-6 relative">
      ...
      {/* GRID LAYOUT: LEFT (Upload/Results) - RIGHT (History) */}
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* === LEFT COLUMN: MAIN ACTION AREA === */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* CONDITION: Show RESULTS if completed */}
          {status === 'completed' && resultData ? (
            <div className="animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">{resultData.title}</h2>
                  <p className="text-sm text-slate-500">Analysis Results</p>
                </div>
                <button onClick={handleReset} className="text-blue-600 hover:bg-blue-50 px-4 py-2 rounded-lg font-medium flex items-center transition">
                  <ArrowLeft className="w-4 h-4 mr-2" /> New Upload
                </button>
              </div>
            ...
            /* CONDITION: Show UPLOAD FORM if idle */
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex gap-3 text-sm">
                      ...
                      <a href="/sample_exam.xlsx" download className="text-slate-500 hover:text-slate-700 flex items-center">
                        <FileQuestion className="w-3 h-3 mr-1" /> Template
                      </a>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-1">
          <ExamList 
            onSelectExam={handleHistorySelect} 
